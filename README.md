# README Template Creator
A small project to ease up the task of writing my repository's README files by copying the same format. Instead of that time consuming task, this project has a template stored and then it replaces everything based on the information entered by the user. Programmed in Python.

## Required Package(s)
Python Version Used: 3.11


```bash
pip install 
```

## Installation Guide
Clone this repo and run the python file!

## Future Plans
1. Add few optional and useful fields.
2. Insert more Formatting options.
3. Required Packages in a more clean format and fix where no packages are used and still  'required packages' still shows up.
4. Create a computer GUI app using Tkinter/Streamlit.
5. Try to establish network between the Native Mobile App & API.
